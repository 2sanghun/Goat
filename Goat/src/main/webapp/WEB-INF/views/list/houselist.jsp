	...
	<div id="content">
		<div id="middle">
			...
			<table>
				...
				<!--  게시글 목록 for문 시작 -->
				<c:forEach items="${list}" var="houselist">
				<tr>
					...
					<td class="good">${houselist.good}</td>
				</tr>
				</c:forEach>
				<!-- 게시글 for문 끝 -->
			</table>
		...
		<div id="search">
			<div id="pagingArea">
				...
				<c:if test="${paging.prev}">
					<a href="/list/houselist?category=${paging.cri.category}&period=${paging.cri.period}&type=${paging.cri.type}&keyword=${paging.cri.keyword}&pageNum=${paging.startPage-1}&amount=${paging.cri.amount}" id="pageBtn">이전</a>
				</c:if>
		
				<!-- begin(1)이 end(10) 될 동안 반복(1이 10 될 동안 반복) -->
				<c:forEach begin="${paging.startPage}" end="${paging.endPage}" var="num">
					<a href="/list/houselist?category=${paging.cri.category}&period=${paging.cri.period}&type=${paging.cri.type}&keyword=${paging.cri.keyword}&pageNum=${num}&amount=${paging.cri.amount}" id="pageNum">${num}</a>
				</c:forEach>
				
				<!-- next(다음)이 true이면 다음버튼 활성화 -->
				<c:if test="${paging.next}">
					<a href="/list/houselist?category=${paging.cri.category}&period=${paging.cri.period}&type=${paging.cri.type}&keyword=${paging.cri.keyword}&pageNum=${paging.endPage+1}&amount=${paging.cri.amount}" id="pageBtn">다음</a>
				</c:if>
			</div>
			<form action="/list/houselist" method="get" id="searchform">
				<div id=searchAll>
